...
object DB {
  ...
  def createTableStmt(tableName: String, addressLength: Int = 500, addressComponentsLength: Int = 200, maxLongTextIndexLength: Int = 100, unformattedAddressUnique: Boolean = true, maxIndexLength: Option[Int] = None): String = {
    ...
    s"""
      ...
      |  ${akka_parser.old_parser.AddressParser.addressComponentTypes.map(c => s"$c varchar($addressComponentsLength)").mkString(", ")},
      ...
      |  ${akka_parser.old_parser.AddressParser.addressComponentTypes.map(c => s"index($c$ls)").mkString(", ")}
...
class DB(dbUrl: String, tableName: String) extends Actor with ActorLogging {
  import DB._
  implicit val conn: Connection = getDbConnection(dbUrl)
  ...
  def receive = {
    ...
    case SaveGoogleResponseAndAddress(id, googleResponse, parsedAddress) =>
      ...
      val params: Seq[NamedParameter] =
        (akka_parser.old_parser.AddressParser.addressComponentTypes.map(t => (t, parsedAddress.addressComponents.get(t))).toMap +
